  ...
  return (
    <div className="bg-card p-3 rounded-lg border border-border/80 space-y-2.5 shadow-xs" ref={containerRef}>
      ...
      <div className="space-y-1">
        ...
        {customerType === 'new' ? (
          // Renders editable field if NEW account is running
          <Input
            className="h-8 text-xs bg-background"
          ...
          // Renders dropdown list if EXISTING account is selected
          <Select value={selectedLocation} onValueChange={handleLocationChange} disabled={!selectedBillToId || locations.length === 0}>
            <SelectTrigger className="h-8 text-xs bg-background">
              <SelectValue placeholder={selectedBillToId ? "Choose site location" : "Select customer first"} />
            </SelectTrigger>
            <SelectContent>
